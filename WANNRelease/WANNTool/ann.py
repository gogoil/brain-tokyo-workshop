...
from model import number_of_inside_iterations
...
def act(weights, aVec, nInput, nOutput, inPattern):
    ...
    print('inside check of number of iterations {}'.format(
        number_of_inside_iterations.value), flush=True)
    for i in range(number_of_inside_iterations.value):
        ...
        print(prev_computation, flush=True)
        # if PRINTING:
        #     print('node act: ')
        #     print(nodeAct)
...
def getNodeOrder(nodeG, connG):
    ...
    Q = np.where(edge_in == 0)[
        0]  # Start with nodes with no incoming connections
