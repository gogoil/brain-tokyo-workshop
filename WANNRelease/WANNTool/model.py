...
K = 100
VALUE_LIST = [[-2, 2], [0.75, 2], [1.75, 3], [2.75, 4], [3.75, 5], [-2, -0.75]]
...
class Model:
    ...
    def __init__(self, game):
        ...
        self.wVec, self.aVec, self.wKey = ann.importNet(
            "champions/" + self.wann_file)
    ...
    def get_action(self, x):
        ...
        annOut = ann.act(self.wVec, self.aVec, self.input_size,
                         self.output_size, x)
    ...
    def modified_get_uniform_random_model_params(self, hi, lo):
        np.random.seed()
        return np.random.rand(self.param_count) * (hi - lo) + lo
...
def evaluate(model):
    ...
    N = 100
    for i in range(N):
        reward, t = simulate(model, train_mode=False, render_mode=False,
                             num_episode=1)
...
def main():
    ...
    parser.add_argument('-f', '--filename', type=str, help='json filename',
                        default='none')
    parser.add_argument('-e', '--eval_steps', type=int, default=100,
                        help='evaluate this number of step if final_mode')
    parser.add_argument('-s', '--seed_start', type=int, default=0,
                        help='initial seed')
    parser.add_argument('-w', '--single_weight', type=float, default=-100,
    ...
    parser.add_argument('--lo', type=float, default=-2.0,
    ...
    for pair in VALUE_LIST:
        for i in range(len(fit_arr)):
            fit_arr[i] = evalute_one_loop(pair[0], pair[1], eval_steps,
                                          filename,
                                          game,
                                          gamename,
                                          model,
                                          num_sweep,
                                          single_weight, sweep_hi, sweep_lo,
                                          the_seed,
                                          use_model,
                                          weight_stdev)
...
def evalute_one_loop(hi, lo, eval_steps, filename, game, gamename, model,
                     num_sweep,
                     single_weight, sweep_hi, sweep_lo, the_seed, use_model,
                     weight_stdev):
    if use_model:
        ...
        if single_weight > -100:
            ...
            params = model.modified_get_uniform_random_model_params(
                hi, lo) - game.weight_bias
            # print('params: ', params)
        model.set_model_params(params)
    if final_mode:
        if num_sweep > 1:
            the_weights = np.arange(sweep_lo, sweep_hi + (
            ...
            for i in range(len(the_weights)):
                ...
                for i in range(eval_steps):
                    reward, steps_taken = simulate(model, train_mode=False,
                                                   render_mode=False,
                                                   num_episode=1,
                                                   seed=the_seed + i)
        ...
        if record_video:
            model.env = Monitor(model.env, directory='/tmp/' + gamename,
