...
class Ind():
    ...
    def __init__(self, conn, node):
        ...
        self.birth = []
        self.species = []
        # can add info from prev computation, for example
    ...
    def express(self):
        ...
        """
        _, wMat = getNodeOrder(self.node, self.conn)
        self.wMat = wMat
...
def getNodeOrder(nodeG, connG):
    ...
    """
    # print('started another round. Horahy!')

    conn = np.copy(connG)
    node = np.copy(nodeG)
    nIns = len(node[0, node[1, :] == 1]) + len(
        node[0, node[1, :] == 4])  ### is 4
    ### the bias?
