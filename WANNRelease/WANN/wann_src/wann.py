...
import networkx_graph_analyzer
from .ind import Ind
...
class Wann():
    ...
    def ask(self):
        ...
        # for ind in self.pop:  # check cycles:
        #     G = networkx_graph_analyzer.create_graph(ind.conn[1, :],
        #                                              ind.conn[2, :])
        #     if networkx_graph_analyzer.is_cycle_exist(G):
        #         print('\n\n\n\n****************we did it****************\n\n\n\n')
        #         print(ind.conn[1, :])
        #         print(ind.conn[2, :])
    ...
    def initPop(self):
        ...
        outs = (p['ann_nInput'] + 1) + np.arange(0, p[
            'ann_nOutput'])  # Output Ids
        ...
        for i in range(p['popSize']):
            ...
            newInd.conn[3,
            :] = 1  # (2*(np.random.rand(1,nConn)-0.5))*p['ann_absWCap']
    ...
    def probMoo(self):
        ...
        # Compile objectives
        meanFit = np.asarray(
            [ind.fitness for ind in self.pop])  # Mean fitness over trials
        maxFit = np.asarray(
            [ind.fitMax for ind in self.pop])  # Best fitness over trials
        nConns = np.asarray(
            [ind.nConn for ind in self.pop])  # model complexity
        nConns[
            nConns == 0] = 1  # No conns is always pareto optimal (but boring)
